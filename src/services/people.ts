...
type GetOneFilters = { id_event: number; id_group?: number; id?: number; cpf?: string; setor?: string; chave?: string; }
...
type UpdateFilters = { id?: number; id_event: number; id_group?: number; setor?: string; chave?: string; }
...
// Adiciona a verificação de CPF
export const cpfExists = async (cpf: string): Promise<boolean> => {
    try {
        const person = await prisma.eventPeople.findFirst({ where: { cpf } });
        return !!person; // Retorna true se a pessoa for encontrada, false caso contrário
    } catch (err) {
        return false;
    }
}
